...
def create_cylinder_two_point(p1, p2, radius=0.1, offset=0.0):
    """This function is used to create a cylinder between two points p1 and p2. The cylinder is created around the z axis. The cylinder is then rotated and translated to the mid point between p1 and p2. The cylinder is then returned."""
    mesh_cylinder = o3d.geometry.TriangleMesh.create_cylinder(
        radius=radius, height=np.linalg.norm(p2 - p1) + offset
    )
    ...
    z = np.array([0, 0, 1]).astype(
        np.float64
    )  # The cylinder is created around the z axis
